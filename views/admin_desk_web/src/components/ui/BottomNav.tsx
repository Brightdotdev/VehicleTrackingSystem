...
if(isValidated && !loading) return (
          <nav className="z-10 backdrop-blur-xs bg-white/1  fixed bottom-0 w-full h-[var(--size-lg)]
        flex items-start justify-center">

            <section className=" flex items-center justify-center w-[16rem]">

<Button 
onClick={() => handleHistoryRoute()} 
className={`bottomNavButton  navButtonLeft
${path === "history" ? "bg-blue-950/90 dark:bg-blue-850/90" : ""}`}>

<History className={`w-[var(--size-xl)]  ${path === "history" ? "dark:stroke-primary" : ""} `}/></Button>

<Button 
onClick={() => handleMapRoute()} 
className={`bottomNavButton rounded-none 
${path === "map" ? "bg-blue-950/90 dark:bg-blue-850/90" : ""}`}>
<Map className={`w-[var(--size-xl)]  ${path === "map" ? "dark:stroke-primary" : ""} `} /></Button>

<Button 
onClick={() => handleVehicleRoute()} 
className={`bottomNavButton  navButtonRight  
${path === "vehicles" ? "bg-blue-950/90 dark:bg-blue-850/90" : ""}`}>
<Car className={`w-[var(--size-xl)]  ${path === "vehicles" ? "dark:stroke-primary" : ""} `} /></Button>
