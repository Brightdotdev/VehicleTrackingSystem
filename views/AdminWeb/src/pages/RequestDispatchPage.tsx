...
const RequestDispatchPage: React.FC = () => {
  ...
  const [requests, setRequests] = useState<VehicleData[]>([]);
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        setLoading(true);
        // TODO: Replace mockApiService.getRequests() with actual API call
        const data = await mockApiService.getRequests();
        setRequests(data);
        setError(null);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 18, color: "#666" }}>Loading requests...</div>
      </div>
    );
  }
