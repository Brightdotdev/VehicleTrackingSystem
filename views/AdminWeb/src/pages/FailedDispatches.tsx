...
const FailedDispatches = () => {
  ...
  useEffect(() => {
    const fetchDispatchHistory = async () => {
      try {
        ...
        setDispatchHistory(data.filter(d => d.status === 'Rejected'));
        setError(null);
      } catch (err) {
  ...
  if (loading) {
    return <div style={{ background: "#fff", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}><div style={{ fontSize: 18, color: "#666" }}>Loading failed/canceled dispatches...</div></div>;
  }
  if (error) {
    ...
    <div style={{ background: "#fff", minHeight: "100vh", width: "100vw", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", margin: 0, padding: 0, boxSizing: "border-box", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 1440, display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 24, marginBottom: 32, paddingLeft: 16, paddingRight: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ color: '#ef4444', fontWeight: 700, fontSize: 22 }}>Failed / Canceled Dispatches</span>
        </div>
      </div>
      {/* Table */}
      <div style={{ width: "100%", maxWidth: 1100, display: "flex", flexDirection: "column", gap: 16, margin: 0, padding: 0 }}>
        ...
        {dispatchHistory.map((item) => (
          <div key={item.id} style={{ width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr 1.2fr 1.2fr", alignItems: "center", background: '#fff6f6', borderRadius: 8, boxShadow: '0 1px 4px #ef444422', marginBottom: 8, padding: '16px 0' }}>
            ...
            <div style={{ textAlign: "center", color: '#ef4444', fontWeight: 600 }}>Rejected</div>
          </div>
        ))}
      </div>
      <BottomNavBar />
...
export default FailedDispatches; 